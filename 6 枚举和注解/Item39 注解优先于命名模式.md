...
处理可重复注解的代码需要非常小心。一个重复的注解会生成一个包含注解类型的合成注解。getAnnotationsByType方法忽略了这个事实，因此可以被用来或者可重复注解类型的重复的和非重复的注解。但是isAnnotationPresent方法却让这个方法变成了显式的，也就是说，重复的注解不是这个注解类型，而是包含注解类型。如果一个元素有一个类型的重复注解，然后你使用isAnnotationPresent方法拉检查这个元素是否有这个类型的注解，你会发现它没有。使用isAnnotationPresent方法来一个注解类型是否存在，会导致你的程序忽略掉重复的注解。同样的，当你使用isAnnotationPresent方法来检测 包含注解类型 是否存在，也会导致程序忽略掉非重复的注解。为了能使用isAnnotationPresent方法来检测重复的和非重复的注解，你必须要对注解类型和它的包含注解类型都进行检查。下面是运行测试程序，改成使用ExceptionTest的可重复版本的相关代码：
